...
jQuery.reel || (function($, window, document, undefined){
  ...
  var
    reel= $.reel= {
      ...
      def: {
        ...
        //
        images:			'',
        zoomimages:		'', //FBZoom Zoom images
	zoomcache:		[], //FBZoom Zoom image already loaded?
	imageno:                 1, //FBZoom Current Frame No.
	zoomno:		      1000, //FBZoom Current Zoom Frame No.
	zoomwidth: 		 0, //FBZoom zoom image width
	zoomheight:		 0, //FBZoom zoom image height
      ...
      fn: {
        ...
        reel: function(){
          ...
          var
            ...
            opt.imagewidth= $(this).width(); //FBZoom Get image width
            opt.imageheight= $(this).height(); //FBZoom Get image height
          ...
          applicable.each(function(){
              ...
              on= {
                ...
                setup: function(e){
                  ...
                  var
                    ...
                    data= $.extend({}, t.data()),
                    images= opt.images,
                    zoomimages= opt.zoomimages, //FBZoom
                ...
                instance: {
                  ...
                  setup: function(e){
                    ...
                    if (touchy){
                      ...
                      $area
                        //FBZoom Deactivate standard wheel function .bind(opt.wheelable ? _mousewheel_ : null, wheel)
			...
			//--> FBZoom Zoom Function
			if (opt.zoomimages[0] !== '' && typeof(opt.zoomimages[0]) !== 'undefined') { 
				t.parent().css({width: opt.imagewidth, height: opt.imageheight, overflow:'hidden'});
				t.parent().wrap('<div class="fb3dwrap" />').css({ width:opt.imagewidth });
				t.css({position: "relative"});
				t.before($slider= $(tag(_div_), { 'id': 'zoomslider' }));
				...
				$slider.slider({
					...
					change: function(event, ui) { 
						if(opt.zoomno !== (opt.imageno - 1)) {
							var hiSrc = opt.zoomimages[opt.imageno - 1];
							swapImage(t, hiSrc);
							opt.zoomno = opt.imageno - 1;
						}

						val = ui.value;
						newWidth = opt.imagewidth+((opt.zoomwidth-opt.imagewidth)*val)/100;
						newHeight = opt.imageheight+((opt.zoomheight-opt.imageheight)*val)/100;
						ig_pos = t.position();
						newLeft = ig_pos.left + ((t.width() - newWidth)/2);
						newTop = ig_pos.top + ((t.height() - newHeight)/2);
						t.css({ width: newWidth, height: newHeight, left:newLeft, top:newTop});
					}

				});

				//Moving zoom image according to mouse movement
				t.parent().mousemove(function(e){
					divWidth = t.parent().width();
					divHeight = t.parent().height();
					igW = t.width();
					igH = t.height();
					dOs = t.parent().offset();
					leftPan = (e.pageX - dOs.left) * (divWidth - igW) / divWidth;
					topPan = (e.pageY - dOs.top) * (divHeight - igH) / divHeight;
					t.css({left: leftPan, top: topPan});			
				});

				//Mousewheel to zoom image
				if(typeof(jQuery.event.special.mousewheel) !== 'undefined') {
					t.parent().mousewheel(function(event, delta) {
						if (delta > 0) {
							valore = parseInt($slider.slider('value'), 10) + 5;
			...
			//<-- FBZoom
                  ...
                  */
                  ...
                  //
                  /*--> FBZoom
                  //Deactivate standard wheel function
                  ...
                  frameChange: function(e, set_frame, frame){
                    ...
                    var
                      ...
                      space= get(_dimensions_)
                    if (!is_sprite){
			//--> FBZoom: Change pictures during Zoom
			if(number($(hash('zoomslider')).slider('value')) > 0 && t.attr('src') !== opt.zoomimages[frame - 1]) {
				//Preload image and then start loading zoom image
				if (opt.zoomcache[frame - 1] !== true) {
					ready && t.attr({ src: reen(opt.path + images[frame - 1]) });
					var hiSrc = reen(opt.path + opt.zoomimages[frame - 1]);
					$imgZoom= $(tag(_img_)).attr({ 'class': cached_klass, width: space.x, height: space.y }).appendTo('#image-reel');
					$imgZoom.attr({ src: hiSrc });
					swapImage(t, hiSrc);
					opt.zoomcache[frame - 1] = true;
				} else {
					t.attr({ src: reen(opt.path + opt.zoomimages[frame - 1]) });
				}
			} else {
			//<-- FBZoom
			      var
				frameshot= images[frame - 1];
			      ready && t.attr({ src: reen(path + frameshot) });
                    ...
                    opt.imageno = frame; //FBZoom Set frame no.
                    opt.zoomno = 1000; //FBZoom Unset Zoom frame no.
                  },
  ...
  //--> FBZoom Function to change image to zoom image
  function swapImage(param, uri){
	param.load().error(function (){
